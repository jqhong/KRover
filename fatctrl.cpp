...
CFattCtrl::CFattCtrl(VMState *VM, EveMeta* meta) {
    ...
    std::string filename = "/home/jqhong/Documents/test_user/kernel_se/ker_func.txt";
...
void CFattCtrl::PoolInit (size_t size)
{
    ...
    m_page_pool->init = temp + 0x1000;//The first page will not encounter #PF, so, start from the second page
    m_page_pool->next = temp + 0x1000;
    m_page_pool->end = temp + size;
